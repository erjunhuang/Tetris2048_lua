...
local PopupManager =  import(".PopupManager")
Panel.SIZE_SMALL = {} 
...
function Panel:showPanel_(isModal, isCentered, closeWhenTouchModel, useShowAnimation, animationArgs, ingoreKind)
    if not self.__popupManager then
        -- 容错处理   防止没有调父类方法
        self.__popupManager = PopupManager
    end
    cc.Director:getInstance():getOpenGLView():setIMEKeyboardState(false);
    PopupManager:addPopup(self, isModal ~= false, isCentered ~= false, closeWhenTouchModel ~= false, useShowAnimation ~= false, animationArgs, ingoreKind)
    return self
end
